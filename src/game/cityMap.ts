...
function avenidasBase(): { id: string; eje: 'v' | 'h'; at: number; desde: number; hasta: number; anillo?: boolean; ramal?: string }[] {
  const lista: { id: string; eje: 'v' | 'h'; at: number; desde: number; hasta: number; anillo?: boolean; ramal?: string }[] = [];
...
export const AVENIDAS = [...avenidasBase(), ...RAMALES];
...
export const DISTRITOS: Distrito[] = [
  ...CENTRO_URBANO.map(([id, nombre, icono, x, z, tipo]) => ({
    id: String(id), nombre: String(nombre), icono: String(icono), x: Number(x), z: Number(z), w: MANZANA, d: MANZANA, tipo: String(tipo),
  })),
  ...ANILLO.map(([id, nombre, icono, x, z]) => ({
    id: String(id), nombre: String(nombre), icono: String(icono), x: Number(x), z: Number(z), w: BANDA, d: BANDA, tipo: 'casas',
  })),
  ...AFUERAS,
];
...
export const CONDOMINIO = {
  ...
  pasaje: { z: 27.5, ancho: 6, desde: 8, hasta: 46 },
  porton: { x: 9.5, z: 27.5 },
  comun: { x: 12.5, z: 16.75, w: 9, d: 15 },
  lotes: [
    { id: 'sur1', nombre: 'Lote 1', x: 25, z: LOTE_SUR, w: 14, d: LOTE_FONDO, venta: true },
    { id: 'sur2', nombre: 'Lote 2', x: 35.5, z: LOTE_SUR, w: 6.5, d: LOTE_FONDO, venta: false },
    { id: 'sur3', nombre: 'Lote 3', x: 42.5, z: LOTE_SUR, w: 6.5, d: LOTE_FONDO, venta: false },
    { id: 'nor1', nombre: 'Lote 4', x: 12.5, z: LOTE_NORTE, w: 6.5, d: LOTE_FONDO, venta: false },
    { id: 'nor2', nombre: 'Lote 5', x: 19.5, z: LOTE_NORTE, w: 6.5, d: LOTE_FONDO, venta: false },
    { id: 'nor3', nombre: 'Lote 6', x: 26.5, z: LOTE_NORTE, w: 6.5, d: LOTE_FONDO, venta: false },
...
export function lotePorId(id: string) { return CONDOMINIO.lotes.find(l => l.id === id) || null; }
export function loteEn(x: number, z: number, margen = 0) {
  for (const l of CONDOMINIO.lotes) {
    if (Math.abs(x - l.x) <= l.w / 2 + margen && Math.abs(z - l.z) <= l.d / 2 + margen) return l;
  }
  return null;
}
export function rectLote(l: { x: number; z: number; w: number; d: number }, margen = 0) {
  return {
...
export function enCondominio(x: number, z: number, margen = 0) {
...
export function rectDistrito(d: Distrito, margen = 0) {
  return {
...
export function rectCalle(av: { eje: string; at: number; desde: number; hasta: number }, margen = 0) {
...
export function seCruzan(a: { minX: number; maxX: number; minZ: number; maxZ: number }, b: { minX: number; maxX: number; minZ: number; maxZ: number }) {
  return a.minX < b.maxX && a.maxX > b.minX && a.minZ < b.maxZ && a.maxZ > b.minZ;
}

export function distritoEn(x: number, z: number, margen = 0) {
...
export function enCalle(x: number, z: number, margen = 0) {
...
export function callesDelMapa() {
  return AVENIDAS.map(av => ({
    ...
    horizontal: av.eje === 'h',
    ramal: !!av.ramal,
    anillo: 'anillo' in av && Boolean(av.anillo),
  }));
}
...
export function manzanasLibres() {
  const libres: { x: number; z: number; w: number; d: number }[] = [];
